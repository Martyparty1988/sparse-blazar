...
const WorkerForm: React.FC<WorkerFormProps> = ({ worker, onClose }) => {
        ...
        for (const project of activeProjects) {
          ...
          if (isAssigned && !wasAssigned) {
            ...
            if (project.firebaseId) {
              firebaseService.updateRecord('projects', project.firebaseId, { workerIds: newWorkerIds }).catch(console.error);
            }
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center bg-[#020617]/90 backdrop-blur-xl p-0 md:p-4 animate-fade-in" onClick={handleClose}>
      <div className="w-full max-h-[95vh] md:max-w-xl bg-[#0a0c1a] rounded-t-[3rem] md:rounded-[3rem] shadow-[0_0_100px_rgba(99,102,241,0.1)] border border-white/5 flex flex-col overflow-hidden animate-slide-up" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-8 py-8 border-b border-white/5 bg-gradient-to-b from-white/[0.02] to-transparent flex justify-between items-center">
          <div className="space-y-1">
            ...
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">{t('worker_settings_subtitle')}</p>
          </div>
          <button onClick={handleClose} className="p-3 text-slate-500 hover:text-white transition-all bg-white/5 rounded-2xl hover:rotate-90 duration-300 active:scale-90">
          ...
          <div className="space-y-6">
            <div className="group relative">
              ...
              <input
                ...
                required
                className="w-full px-6 py-5 bg-black/40 text-white text-lg font-bold rounded-3xl border border-white/5 focus:border-indigo-500/50 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none min-h-[64px]"
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2 p-5 bg-black/40 rounded-3xl border border-white/5">
                {activeProjects?.length === 0 ? (
                  <p className="text-[10px] font-bold text-slate-600 uppercase italic">{t('no_active_projects')}</p>
                ) : (
                  activeProjects?.map(project => (
                    <button
                      ...
                      className={`group relative flex items-center gap-2 px-5 py-3 rounded-2xl text-[11px] font-black uppercase tracking-tight transition-all duration-300 border active:scale-95 ${projectIds.includes(project.id!)
            ...
            {user?.role === 'admin' && (
              <div className="space-y-6 pt-6 border-t border-white/5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-[9px] font-black text-indigo-500/50 uppercase tracking-[0.3em]">{t('financial_settings')}</span>
                  <div className="h-px flex-1 bg-white/5"></div>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  {[
                    { id: 'rate', label: t('hourly_rate') + ' (€)', val: hourlyRate, set: setHourlyRate },
                    { id: 'panel', label: t('panel_price') + ' (€)', val: panelPrice, set: setPanelPrice },
                    { id: 'string', label: t('string_price') + ' (€)', val: stringPrice, set: setStringPrice },
                    { id: 'meter', label: t('meter_price') + ' (€)', val: meterPrice, set: setMeterPrice }
                  ].map(f => (
                    <div key={f.id} className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full px-5 py-4 bg-black/40 text-white font-black rounded-2xl border border-white/5 focus:border-emerald-500/50 transition-all outline-none min-h-[56px]"
                      />
                    </div>
                  ))}
                </div>
                ...
                <div className="pt-4">
                  ...
                  <input
                    ...
                    className="w-full px-6 py-5 bg-black/40 text-white font-bold rounded-3xl border border-white/5 focus:border-indigo-500/50 outline-none min-h-[64px]"
                    placeholder="••••••••"
                  />
                </div>
              </div>
            ...
            <div className="pt-6 border-t border-white/5">
              ...
              <div className="flex flex-wrap gap-4 p-5 bg-black/40 rounded-[2.5rem] border border-white/5 justify-center">
                {['#3b82f6', '#6366f1', '#8b5cf6', '#d946ef', '#ef4444', '#f59e0b', '#22c55e', '#10b981', '#06b6d4', '#4f46e5', '#db2777', '#f97316'].map((c) => (
                  <button
                    key={c}
                    type="button"
                    onClick={() => handleSetColor(c)}
                    className={`h-12 w-12 rounded-2xl ${color === c ? 'ring-4 ring-white ring-offset-4 ring-offset-[#0a0c1a] scale-110 shadow-[0_0_20px_rgba(255,255,255,0.2)]' : 'opacity-40 hover:opacity-100'} transition-all duration-300 active:scale-90`}
        ...
        <div className="p-8 border-t border-white/5 bg-white/[0.01] flex flex-col md:flex-row gap-4 pb-safe md:pb-8">
          <button
            type="button"
            onClick={handleClose}
            className="px-10 py-5 bg-white/5 text-slate-400 font-black rounded-[2rem] hover:bg-white/10 hover:text-white transition-all uppercase tracking-widest text-[11px] min-h-[64px] active:scale-95"
          >
            {t('cancel')}
          </button>
          <button
            ...
            className="flex-1 px-12 py-5 bg-white text-black font-black rounded-[2rem] hover:bg-indigo-600 hover:text-white transition-all shadow-[0_15px_30px_rgba(255,255,255,0.1)] uppercase tracking-[0.2em] text-[11px] min-h-[64px] active:scale-95"
          >
            {t('save')}
          </button>
        </div>
